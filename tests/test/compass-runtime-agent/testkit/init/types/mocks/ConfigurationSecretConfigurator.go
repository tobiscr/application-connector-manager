...
func (_m *ConfigurationSecretConfigurator) Do(configurationSecretName string, config types.CompassRuntimeAgentConfig) (types.RollbackFunc, error) {
	...
	var r1 error
	if rf, ok := ret.Get(0).(func(string, types.CompassRuntimeAgentConfig) (types.RollbackFunc, error)); ok {
		return rf(configurationSecretName, config)
	}
...
func NewConfigurationSecretConfigurator(t interface {
	mock.TestingT
	Cleanup(func())
