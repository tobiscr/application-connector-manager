...
| appConnValidator.logLevel | Determines the level of verbosity or detail for logging messages generated by the central Application Connectivity Validator; one of `debug`, `panic`, `fatal`, `error`, `warn`, `info`, `debug` | false |
| appConnValidator.logFormat | Allows for specifying the format in which the central Application Connectivity Validator generates log messages; one of: `json`, `text`  | false |
...
| appGateway.proxyTimeout | Specifies the maximum duration for which the central Application Gateway waits for a response from an external system before timing out | false |
| appGateway.requestTimeout | Specifies the maximum duration for which the central Application Gateway waits for a response from a client request before timing out. | false |
